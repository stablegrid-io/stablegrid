...
export function SolarForecastingModelPreview({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const checkAvailability = async () => {
      try {
        const response = await fetch(SOLAR_MODEL_URL, {
          method: 'HEAD',
          cache: 'no-store'
        });
        if (!cancelled) {
  ...
  if (available === null) {
    return (
      <div
        className={`h-24 animate-pulse rounded-lg border border-[#2b4f41] bg-[radial-gradient(circle_at_24%_16%,rgba(88,132,255,0.2),transparent_42%),#08120f] ${className ?? ''}`}
      />
    );
  }
