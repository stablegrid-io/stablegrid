...
export function SocialAuthButtons({ onError }: SocialAuthButtonsProps) {
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    try {
      ...
      const message = error instanceof Error ? error.message : `Failed to continue with ${provider}.`;
      onError?.(message);
    }
  };
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <button
        ...
        className="inline-flex items-center justify-center gap-2 border border-on-surface bg-surface px-4 py-3 font-ui-label text-[12px] uppercase tracking-wider text-on-surface transition-colors hover:bg-surface-container"
      >
        <GoogleIcon />
        Google
      </button>
