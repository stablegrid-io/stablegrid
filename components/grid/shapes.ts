 ...
 * category color), outer fill is cream so the shape reads as an outlined
 * pip on the editorial basemap. The inner `.grid3d-shape-fill` polygon is
 * hidden by default and becomes opaque when the parent has `is-deployed`,
...
export function categoryShapeMarkup(category: ComponentCategory): string {
  switch (category) {
      ...
      return `
        <svg class="grid3d-shape" viewBox="0 0 32 32" aria-hidden="true">
          <polygon points="8,4 24,4 30,16 24,28 8,28 2,16"
            fill="${CREAM_FILL}" stroke="currentColor" stroke-width="2.2"
    ...
    case 'balancing':
      return `
        <svg class="grid3d-shape" viewBox="0 0 32 32" aria-hidden="true">
          <circle cx="16" cy="16" r="13" fill="${CREAM_FILL}"
            stroke="currentColor" stroke-width="2.2" vector-effect="non-scaling-stroke"/>
