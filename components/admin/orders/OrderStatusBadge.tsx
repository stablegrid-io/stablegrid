...
const TONE_BY_STATUS: Record<OrderStatus, string> = {
  ...
  Cancelled: 'border-rose-300/24 bg-rose-500/12 text-rose-100'
};

export function OrderStatusBadge({ status }: { status: OrderStatus }) {
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium ${TONE_BY_STATUS[status]}`}
    >
      {status}
    </span>
  );
