    ...
    <div className="overflow-hidden rounded-[1.75rem] border border-[#ddd3c4] bg-[rgba(255,249,242,0.86)] shadow-[0_18px_48px_-38px_rgba(17,24,39,0.22)] backdrop-blur (10,18,14,0.74)]">
      <div className="p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-[#121b18] #f2f7f4]">
              Practice today
            </h2>
            <p className="mt-0.5 text-xs text-[#6d746f] #7e9589]">
              {done
                ? `Goal reached. ${questionsToday} questions answered today.`
                : `${questionsToday} of ${goalPerDay} questions answered today`}
            </p>
          </div>
          <span className="rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary-dim">
            {pct}%
          </span>
        </div>
        ...
        <Target
          className={`mb-4 h-5 w-5 ${
            done ? 'text-amber-500' : 'text-primary-dim '
          }`}
        />
        ...
        {weakestTopic && !done ? (
          <div className="mb-4 rounded-[1.15rem] border border-[#ebe2d5] bg-[rgba(255,255,255,0.72)] p-3 text-xs (255,255,255,0.03)]">
            <span className="mr-1">{getHomeTopicMeta(weakestTopic.topic).icon}</span>
            <span className="text-[#6d746f] #7e9589]">
              Recommended topic:{' '}
            </span>
            <span className="font-medium text-[#28312d] #dbe7e0]">
          ...
          <div className="mb-4 rounded-[1.15rem] border border-[#ebe2d5] bg-[rgba(255,255,255,0.72)] p-3 text-xs text-[#6d746f] (255,255,255,0.03)] #7e9589]">
            {done
        ...
        <Link
          href={practiceHref}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#121b18] py-3 text-sm font-medium text-on-surface transition-colors hover:bg-[#0b120f] #08110b]"
        >
